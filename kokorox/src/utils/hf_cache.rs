...
pub async fn get_available_voices_from_hf(
...
pub async fn download_model_variant(
    model_type: Option<&str>,
    variant: ModelVariant,
) -> Result<PathBuf, Box<dyn std::error::Error + Send + Sync>> {
    let api = Api::new()?;
...
pub async fn download_voice_variant(
    voice_name: &str,
    variant: ModelVariant,
) -> Result<PathBuf, Box<dyn std::error::Error + Send + Sync>> {
    let api = Api::new()?;
...
pub async fn download_and_create_voices_file(
    voice_names: Vec<&str>,
) -> Result<PathBuf, Box<dyn std::error::Error + Send + Sync>> {
...
async fn download_single_voice(
    voice_name: String,
    variant: ModelVariant,
...
pub async fn download_and_create_voices_file_variant(
    voice_names: Vec<&str>,
    variant: ModelVariant,
) -> Result<PathBuf, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let mut voice_data_map: std::collections::HashMap<String, Vec<u8>> =
    ...
    println!(
        "Successfully downloaded {} voices, creating NPZ file...",
        voice_data_map.len()
    );
    ...
    for (i, voice_name) in sorted_names.iter().enumerate() {
        ...
        if voice_data.len() != expected_size {
            eprintln!(
                "Warning: Voice file {} has incorrect size: {} bytes (expected {}), skipping",
                voice_name,
                voice_data.len(),
                expected_size
            );
...
pub async fn download_default_voices_variant(
    variant: ModelVariant,
) -> Result<PathBuf, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn ensure_files_available(
    ...
    ensure_files_available_variant(
        custom_model_path,
        custom_voices_path,
        model_type,
